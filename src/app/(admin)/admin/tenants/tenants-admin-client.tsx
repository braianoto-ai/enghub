...
export function TenantsAdminClient({ tenants }: { tenants: Tenant[] }) {
  ...
  return (
    <div>
      ...
      <p className="mb-3 text-sm text-gray-500 dark:text-slate-400">{filtered.length} de {tenants.length} perfis</p>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-12 text-center text-sm text-gray-400 dark:text-slate-500">Nenhum perfil encontrado</td>
              </tr>
            ) : (
              filtered.map((t) => (
                <tr key={t.id} className={`hover:bg-gray-50 dark:hover:bg-slate-800/50 ${loadingId === t.id ? "opacity-50" : ""}`}>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 dark:text-slate-500">{t.owner_email ?? `/${t.slug}`}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="relative">
                      <button
                        ...
                        className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800"
                      >
                        Ações ▾
                      </button>

                      {openMenuId === t.id && (
                        <div className="absolute right-0 top-8 z-20 w-52 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-900">
                          <div className="border-b border-gray-100 px-3 py-2 dark:border-slate-800">
                            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-slate-500">Alterar plano</p>
                          </div>
                          {PLANS.map((plan) => (
                          ...
                          <div className="border-t border-gray-100 dark:border-slate-800">
                            <button onClick={() => update(t.id, { featured: !t.featured })}
                              className={`flex w-full items-center gap-2 px-3 py-2 text-sm transition-colors ${t.featured ? "text-yellow-600 hover:bg-yellow-50 dark:text-yellow-400 dark:hover:bg-yellow-900/20" : "text-gray-700 hover:bg-gray-50 dark:text-slate-300 dark:hover:bg-slate-800"}`}>
