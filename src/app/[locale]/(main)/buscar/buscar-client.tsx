    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="mb-4 flex gap-3">
        <div className="relative flex-1">
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -tranzinc-y-1/2 text-gray-400 hover:text-gray-600">
              <X size={14} />
            </button>
          )}
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <ChevronDown size={14} className="pointer-events-none absolute right-2.5 top-1/2 -tranzinc-y-1/2 text-gray-400" />
              </div>
            </div>
...
function ProfessionalCard({ professional: p }: { professional: Professional }) {
  ...
  return (
    <Link
      href={`/${p.slug}`}
      className={`group relative flex flex-col rounded-xl border bg-white p-5 shadow-sm transition-all hover:-tranzinc-y-0.5 hover:shadow-md dark:bg-zinc-900 ${
        p.featured
