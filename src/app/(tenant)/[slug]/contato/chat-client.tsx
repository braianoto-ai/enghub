      ...
      <div className="space-y-6">
        ...
        {(whatsapp || phone || website) && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            <h3 className="mb-3 text-sm font-semibold text-zinc-900 dark:text-zinc-100">Contato direto</h3>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          <h3 className="mb-1 font-semibold text-zinc-900 dark:text-zinc-100">Enviar mensagem</h3>
          <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">Inicie uma conversa — o profissional responderá em breve.</p>
          ...
          <form onSubmit={handleStart} className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Seu nome"
                  className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" />
              </div>
              <div>
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Mensagem *</label>
              <textarea value={firstMessage} onChange={(e) => setFirstMessage(e.target.value)} required rows={4}
                ...
                className="w-full resize-none rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" />
            </div>
            <button type="submit" disabled={starting}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 py-3 text-sm font-semibold text-white transition-colors hover:bg-violet-500 disabled:opacity-60">
              {starting ? <Loader2 size={16} className="animate-spin" /> : <Send size={15} />}
              {starting ? "Enviando..." : "Iniciar conversa"}
            </button>
          </form>
        </div>
      </div>
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 flex flex-col" style={{ height: "calc(100vh - 240px)", minHeight: "480px" }}>
      ...
      <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
        <div className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-violet-700 text-sm font-bold text-white overflow-hidden">
          {avatarUrl ? (
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.sender === "visitor" ? "justify-end" : "justify-start"}`}>
            {msg.sender === "professional" && (
              <div className="relative mr-2 flex h-7 w-7 shrink-0 items-center justify-center self-end rounded-full bg-gradient-to-br from-violet-500 to-violet-700 text-xs font-bold text-white overflow-hidden">
                {avatarUrl ? (
                  <Image src={avatarUrl} alt={tenantName} fill className="object-cover" />
                ) : (
                  initials
            ...
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${
              ...
              <p className={`mt-1 text-[10px] ${msg.sender === "visitor" ? "text-violet-200" : "text-zinc-400"}`}>
      ...
      <div className="border-t border-zinc-100 p-3 dark:border-zinc-800">
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="flex-1 resize-none rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"
